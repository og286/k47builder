    ...
    for j in range(idx,min(nxt,len(lines))):
        if re.match(rf'^\s*{header}\b',lines[j]):
            for k in range(j+1,min(nxt,len(lines))):
                if re.match(r'^\s*(SQUAD UPGRADES|TEAM UPGRADES|OPTIONS|SPECIAL RULES|RIFT UNIT)\b',lines[k]): return j,k
...
def parse_upgrades(idx,nxt):
    b=block(idx,nxt,'SQUAD UPGRADES') or block(idx,nxt,'TEAM UPGRADES')
    if not b: return []
    s,e=b; ups=[]
...
json.dump(out,open(f'{BASE}/{FID}_options_draft.json','w'),indent=2,ensure_ascii=False)
print('units with options:',sum(1 for r in out if r['options']),'/',len(out),
      '| with upgrades:',sum(1 for r in out if r['upgrades']))
