...
SKIP={'SPECIAL RULES','OPTIONS','RIFT UNIT','HERO STATISTICS','ARMY SPECIAL RULES','OFFICERS','COMPANY COMMANDERS','PLATOON COMMANDERS','INFANTRY','MACHINE GUN TEAMS','FLAMETHROWER TEAMS','FIELD ARTILLERY','ANTI-TANK GUNS','SQUAD UPGRADES','ANTI-TANK TEAMS','SNIPER TEAMS','MORTAR TEAMS','ARTILLERY','WALKERS','TANKS','TRANSPORTS','HEROES','SCOUT WALKERS','LIGHT WALKERS','MEDIUM WALKERS','HQ TEAMS','HQ','TEAM UPGRADES','HEAVY WALKERS','ARMOURED CARS','TANK DESTROYERS','ASSAULT GUNS','SELF-PROPELLED ARTILLERY'}
def clean_name(s):
    ...
    for j in range(max(idx-5,0),min(nxt,idx+6)):
        if 'REQUISITION POINTS' in lines[j]:
            for k in range(j,min(j+5,nxt)):
                ...
                if m and (any(x in lines[k] for x in('Composition','Unit Type','Base Size','Standard Weapon','Movement Rate')) or norm(re.sub(r'\d+\s*$','',lines[k]))==''):
...
json.dump(units,open(f'{BASE}/{FID}_units_draft.json','w'),indent=2)
