...
def stop_line(s):
    return any(l in s for l in FIELDLABELS) or re.match(r'^\s*(SQUAD UPGRADES|OPTIONS|SPECIAL RULES|RIFT UNIT)\b',s) or 'REQUISITION POINTS' in s
def strip_cost(s):
...
def parse_special(lo,hi):
    ...
    for j in range(lo,hi):
        if re.match(r'^\s*SPECIAL RULES\b',lines[j]):
            ...
            while k<hi:
                s=lines[k]
                if re.match(r'^\s*(RIFT UNIT|OPTIONS|SQUAD UPGRADES)\b',s) or 'REQUISITION' in s: break
                if '•' in s:
                    t=norm(s.split('•',1)[1])
                    ...
                    while (kk<hi and '•' not in lines[kk] and norm(lines[kk])!='' and
                           not re.match(r'^\s*(RIFT UNIT|OPTIONS|SQUAD UPGRADES)\b',lines[kk]) and
...
def parse_hero_stats(lo,hi):
    ...
    for j in range(lo,hi):
        if re.match(r'^\s*SPECIAL RULES\b',lines[j]):
            for k in range(j,hi):
                if re.match(r'^\s*(RIFT UNIT|OPTIONS|SQUAD UPGRADES)\b',lines[k]) or 'REQUISITION' in lines[k]: break
...
json.dump(stats,open('/sessions/eloquent-epic-mendel/mnt/outputs/axis_stats_draft.json','w'),indent=2,ensure_ascii=False)
for i,u in enumerate(draft):
    nm=u['name'] or ''
    if 'Slammer' in nm or 'Roberta Wells' in nm or nm in ('Paragon Squad','Firefly Airborne Squad'):
        print('###',nm,'| rules:',[r['name'] for r in stats[i]['rules']],'| hero_stats:',stats[i]['hero_stats'])
